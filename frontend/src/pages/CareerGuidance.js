...
function CareerGuidance() {
  ...
  const [formData, setFormData] = useState({
    title: "", description: "", color: "#10b981", link: "#", image: null, category: "guidance"
  });
  ...
  const fetchCareers = async () => {
    try {
      ...
      setCareers(all.filter(c => c.category === "guidance"));
  ...
  const openAddModal = () => { setEditingId(null); setFormData({ title: "", description: "", image: null, color: "#10b981", link: "#", category: "guidance" }); setShowModal(true); };
  const openEditModal = (c) => { setEditingId(c._id); setFormData({ title: c.title, description: c.description, image: null, color: c.color, link: c.link || "#", category: c.category || "guidance" }); setShowModal(true); };
      ...
      <section className="hero hero--career">
        <div className="container hero__container">
          <div className="hero__content">
            <h1 className="hero__title">Career Guidance</h1>
            <p className="hero__subtitle">Get personalized career advice, roadmaps, and mentorship from industry professionals</p>
          </div>
        </div>
      </section>
      ...
      <main className="section">
        <div className="container">
          <div className="section__head">
            <span className="section__tag section__tag--career">Guidance</span>
            <h2 className="section__title">Career Guidance & Mentorship</h2>
            <p className="section__desc">Navigate your career path with expert advice and structured roadmaps</p>
          </div>

          {isAdmin && (
            <div className="career__admin-bar">
              <p style={{color:"#64748b",fontSize:"0.9rem"}}>Admin Mode — {careers.length} guidance items</p>
              <button className="career__addBtn" onClick={openAddModal}>+ Add Guidance Item</button>
            </div>
          )}

          {loading ? (
            <div className="career__loading">Loading career guidance...</div>
          ) : (
            <div className="career__grid">
              {careers.map(c => (
                <article className="career__card" key={c._id}>
                  <div className="career__card-bar" style={{background: c.color || "#10b981"}}></div>
                  {isAdmin && (
              ...
              {careers.length === 0 && (
                <div className="career__empty">
                  ...
                  <p className="career__empty-text">No career guidance articles posted yet.{isAdmin ? " Click '+ Add Guidance Item' to add one." : ""}</p>
                </div>
              )}
            </div>
          )}
      ...
      {showModal && (
        <div className="career__modal-overlay" onClick={()=>setShowModal(false)}>
          <div className="career__modal" onClick={e=>e.stopPropagation()}>
            <h2 className="career__modal-title">{editingId?"Edit Guidance":"Add New Guidance"}</h2>
            <form className="career__form" onSubmit={handleSubmit}>
              <div className="career__form-group"><label>Title</label><input name="title" value={formData.title} onChange={handleInputChange} required placeholder="e.g. How to Build a Career in AI"/></div>
              <div className="career__form-group"><label>Description</label><textarea name="description" value={formData.description} onChange={handleInputChange} required rows="3" placeholder="Describe the guidance content..."/></div>
