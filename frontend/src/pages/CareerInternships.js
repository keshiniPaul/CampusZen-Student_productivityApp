...
function CareerInternships() {
  ...
  useEffect(() => {
    ...
    const onClick = (e) => {
      ...
      if (!navLinksRef.current.contains(e.target) && !navToggleRef.current.contains(e.target))
        setIsNavOpen(false);
    };
  ...
  const fetchCareers = async () => {
    try {
      ...
      setCareers(all.filter(c => c.category === "internship"));
      ...
      <section className="hero hero--career">
        <div className="container hero__container">
          <div className="hero__content">
            <h1 className="hero__title">Internship Opportunities</h1>
            <p className="hero__subtitle">Browse and apply for the latest internship openings from top companies and startups</p>
          </div>
        </div>
      </section>
      ...
      <main className="section">
        <div className="container">
          <div className="section__head">
            <span className="section__tag section__tag--career">Internships</span>
            <h2 className="section__title">Available Internship Positions</h2>
            <p className="section__desc">Find your next career opportunity and kickstart your professional journey</p>
          </div>

          {isAdmin && (
            <div className="career__admin-bar">
              <p style={{color:"#64748b",fontSize:"0.9rem"}}>Admin Mode — {careers.length} internship items</p>
              <button className="career__addBtn" onClick={openAddModal}>+ Add Internship</button>
            </div>
          )}

          {loading ? (
            <div className="career__loading">Loading internship opportunities...</div>
          ) : (
            <div className="career__grid">
              {careers.map(c => (
                <article className="career__card" key={c._id}>
                  <div className="career__card-bar" style={{background: c.color || "#667eea"}}></div>
                  {isAdmin && (
              ...
              {careers.length === 0 && (
                <div className="career__empty">
                  ...
                  <p className="career__empty-text">No internship opportunities posted yet.{isAdmin ? " Click '+ Add Internship' to add one." : ""}</p>
                </div>
              )}
            </div>
          )}
...
export default CareerInternships;
