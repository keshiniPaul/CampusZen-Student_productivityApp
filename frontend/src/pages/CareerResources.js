...
function CareerResources() {
  ...
  const [formData, setFormData] = useState({
    title: "", description: "", color: "#f59e0b", link: "#", image: null, category: "resource"
  });
  ...
  const fetchCareers = async () => {
    try {
      ...
      setCareers(all.filter(c => c.category === "resource"));
  ...
  const openAddModal = () => { setEditingId(null); setFormData({ title: "", description: "", image: null, color: "#f59e0b", link: "#", category: "resource" }); setShowModal(true); };
  const openEditModal = (c) => { setEditingId(c._id); setFormData({ title: c.title, description: c.description, image: null, color: c.color, link: c.link || "#", category: c.category || "resource" }); setShowModal(true); };
      ...
      <main className="section">
        <div className="container">
          <div className="section__head">
            <span className="section__tag section__tag--career">Resources</span>
            <h2 className="section__title">Tools & Materials</h2>
            <p className="section__desc">Everything you need to prepare for your professional future</p>
          </div>

          {isAdmin && (
            <div className="career__admin-bar">
              <p style={{color:"#64748b",fontSize:"0.9rem"}}>Admin Mode — {careers.length} resource items</p>
              <button className="career__addBtn" onClick={openAddModal}>+ Add Resource</button>
            </div>
          )}

          {loading ? (
            <div className="career__loading">Loading resources...</div>
          ) : (
            <div className="career__grid">
              {careers.map(c => (
                <article className="career__card" key={c._id}>
                  <div className="career__card-bar" style={{background: c.color || "#f59e0b"}}></div>
                  {isAdmin && (
              ...
              {careers.length === 0 && (
                <div className="career__empty">
                  ...
                  <p className="career__empty-text">No resources posted yet.{isAdmin ? " Click '+ Add Resource' to add one." : ""}</p>
                </div>
              )}
            </div>
          )}
      ...
      {showModal && (
        <div className="career__modal-overlay" onClick={()=>setShowModal(false)}>
          <div className="career__modal" onClick={e=>e.stopPropagation()}>
            <h2 className="career__modal-title">{editingId?"Edit Resource":"Add New Resource"}</h2>
            <form className="career__form" onSubmit={handleSubmit}>
              <div className="career__form-group"><label>Title</label><input name="title" value={formData.title} onChange={handleInputChange} required placeholder="e.g. CV Template for IT Students"/></div>
              <div className="career__form-group"><label>Description</label><textarea name="description" value={formData.description} onChange={handleInputChange} required rows="3" placeholder="Describe the resource..."/></div>
